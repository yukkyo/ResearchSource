...
# digamma 関数はc++ライブラリのdigammaから呼び出す
...
class LDA:
	...
	def __init__(self, r_n_topics, r_alpha, r_beta, raw_docs, r_V, r_iteration):
		...
		n_z_t_tmp = n_z_t
		log_per = 0.0
		for v in prange(V_int, nogil=True):
			for j in range(n_topics_int):
				n_z_t_tmp[v][j] = (n_z_t_tmp[v][j] + beta) / (n_z[j] + Vbeta)
		sum_alpha = 0.0
		for j in xrange(n_topics_int):
			sum_alpha += alphas[j]
		for m in prange(max_docs, nogil=True):
			len_doc = docs_len[m]
		...
		for ite in xrange(iteration):
			...
			psi_sum_alphas = digamma(sum_alpha)
			psi_sum_alphas -= sum_alpha
			for m in xrange(max_docs):
				for j in xrange(n_topics_int):
					numerator_alpha[j] += digamma(n_m_z[m][j] + alphas[j])
					denominator_alpha[j] += digamma(<double>docs_len[m] - psi_sum_alphas)
			print "end alphas"
			for j in xrange(n_topics_int):
				numerator_alpha[j] -= digamma(alphas[j]) * <double>max_docs
			...
			for v in xrange(V_int):
				for j in xrange(n_topics_int):
					numerator_beta += digamma(n_z_t[j][v] + beta)
			numerator_beta -= digamma(beta) * V * n_topics

			denominator_beta = digamma(n_corpus_double + n_topics * beta) - digamma(Vbeta)
