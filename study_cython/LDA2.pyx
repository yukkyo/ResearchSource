...
class LDA:
	...
	def inference(self):
		...
		cdef int len_doc, m, n, v, new_z, j
		...
		cdef double p_z2j, u
		cdef vector[int] docs_lens = self.docs_lens

		# make random numbers
		cdef vector[vector[vector[double]]] rand_nums
		cdef vector[vector[double]] rand_nums_m
		rand_nums.resize(max_docs)
		for m in xrange(max_docs):
			len_doc = docs_lens[m]
			rand_nums[m] = vector[vector[double]](len_doc, vector[double](n_topics))
			for n in xrange(len_doc):
				for j in xrange(n_topics):
					rand_nums[m][n][j] = (rand()/(RAND_MAX +1.))

		# sampling each word in corpus
		for m in xrange(max_docs):
			len_doc = docs_lens[m]
			rand_nums_m = rand_nums[m]
			n_m_z_m = n_m_z[m]
			z_m_n_m = z_m_n[m]
			for n in xrange(len_doc):
				v = docs[m][n]
	...
	def perplexity(self):
		...
		cdef vector[double] tmp_phi
		tmp_phi.resize(n_topics)
		...
		return exp(log_per / self.n_corpus)
